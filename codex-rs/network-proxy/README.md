...
`codex-network-proxy` is Hoondex's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from Hoondex's merged `config.toml` (via `codex-core` config loading).
...
When `codex-network-proxy` is embedded in managed Hoondex runtime, policy decisions emit structured
