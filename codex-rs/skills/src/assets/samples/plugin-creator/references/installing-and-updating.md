...
- the user wants Hoondex to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single Hoondex cachebuster suffix:
...
outside Hoondex depends on a specific token:
...
`~/.agents/plugins/marketplace.json`. You do not need `hoondex plugin marketplace add` for that
path, and `hoondex plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
6. Prompt the user to use a new thread to try the updated plugin, so that Hoondex picks up new skills
   and tools.
...
Replace the existing Hoondex cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `hoondex plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `hoondex plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Hoondex.
...
- Use `hoondex plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `hoondex plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `hoondex plugin add <plugin-name>@<marketplace-name>`.
