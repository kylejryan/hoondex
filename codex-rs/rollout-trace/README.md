...
> **Privacy:** Rollout tracing is not telemetry. Hoondex does **not** upload or
...
during a Hoondex session. It records raw runtime evidence into a local bundle on
...
Hot-path Hoondex code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Hoondex got
there.
...
Hoondex session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `hoondex debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Hoondex rollout/session being observed. Keeping those separate lets us reason about
