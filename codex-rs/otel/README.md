...
`codex-otel` is the OpenTelemetry integration crate for Hoondex. It provides:
...
that member. If propagated trace context already has the named member, Hoondex
...
Hoondex-specific session events. Rich session/business events should go through
...
to Statsig using Hoondex-internal defaults.
