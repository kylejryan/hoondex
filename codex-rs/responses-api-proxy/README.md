...
A non-privileged user would then run Hoondex as follows, specifying the `model_provider` dynamically:
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the Hoondex CLI expectations.
