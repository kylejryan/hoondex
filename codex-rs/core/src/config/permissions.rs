...
/// for Hoondex to function. These should always be added to the
...
// Adding a new `:special_path` must not make older Hoondex versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Hoondex. Filesystem access will remain restricted. Upgrade Hoondex if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of Hoondex and will be ignored. Upgrade Hoondex if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Hoondex and will be ignored. Upgrade Hoondex if this path is required."
            ),
        },
    );
}
