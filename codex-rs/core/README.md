...
This crate implements the business logic for Hoondex. It is designed to be used by the various Hoondex UIs written in Rust.
...
Expects the binary containing `codex-core` to run the equivalent of `hoondex sandbox` when `arg0` is `codex-linux-sandbox`. See the `codex-arg0` crate for details.
...
binary shipped with Hoondex and Hoondex surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Hoondex also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Hoondex rejects sandboxed shell commands that would enter the
