...
This guide gets a published Hoondex Python SDK beta installation running
...
- Python `>=3.10`
- An existing Hoondex account session, or one of the login flows below
...
Existing Hoondex authentication is reused automatically. For ChatGPT browser
login:
...
When `sandbox=` is omitted, Hoondex uses its configured default. A turn override
...
```python
import openai_codex
from openai_codex import Hoondex, CodexConfig
