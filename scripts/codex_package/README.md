...
The builder creates a canonical Hoondex package directory:
...
host platform. On Linux, that default is a musl target to match Hoondex release
...
`hoondex` and `codex-app-server`. The `version` field in `codex-package.json` is
...
downloads and verifies the matching Hoondex-built V8 release pair before invoking
